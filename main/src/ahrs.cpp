 ...
 */
Adafruit_BNO055::Adafruit_BNO055(uart_port_t uart_port) {
 ...
 *            OPERATION_MODE_NDOF]
 *  @param tx_num the tx gpio pin
 *  @param rx_num the rx gpio pin
 *  @return true if process is successful
 */
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode, gpio_num_t tx_num, gpio_num_t rx_num) {
  ...
  ESP_ERROR_CHECK(uart_set_pin(uart_port, tx_num, rx_num, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
...
/*!
 *  @brief  Writes an 8 bit value over UART
 */
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, uint8_t value) {
  ...
  uint8_t buf[130];
  int avail = uart_read_bytes(uart_port, buf, 2, 100);
  if (avail > 0) {
    if (buf[0] == 0xEE) {
...
/*!
 *  @brief  Reads an 8 bit value over UART
 */
...
/*!
 *  @brief  Reads the specified number of bytes over UART
 */
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t *buffer,
                              uint8_t len) {
  uint8_t packet[] = {0xAA, 0x01, reg, len};
  uart_flush(this->uart_port);
  uart_write_bytes(this->uart_port, (const char*)packet, 4);
  size_t available = 0;
  do {
    uart_get_buffered_data_len(this->uart_port, &available);
    delay(1);
  } while (!available);
  uint8_t buf[130];
  int avail = uart_read_bytes(this->uart_port, buf, available, 100);
  if (avail > 0) {
    if (buf[0] == 0xBB) {
      //uint8_t recvd = buf[1];
